...
def _j1_splits(metadata: pd.DataFrame) -> list[Split]:
    splits: list[Split] = []
    source_dataset_keys = _composite_group_series(metadata, "source_dataset_id")
    distinct_sources = _valid_group_values(source_dataset_keys)
    if len(distinct_sources) > 1:
        for source_key in distinct_sources:
            train_idx = metadata.index[source_dataset_keys != source_key].to_list()
            test_idx = metadata.index[source_dataset_keys == source_key].to_list()
            if train_idx and test_idx:
                splits.append(
                    Split(
                        split_id=f"j1_source_dataset_holdout|{source_key}",
                        train_index=train_idx,
                        test_index=test_idx,
                        train_groups={"source_dataset_id": set(source_dataset_keys.iloc[train_idx])},
                        test_groups={"source_dataset_id": {source_key}},
                    )
                )

    subject_keys = _composite_group_series(metadata, "subject_id")
    group_values = _metadata_string_series(metadata, "group").str.lower()
    amputee_mask = group_values.isin({"amputee", "transradial_amputee"})
    able_mask = group_values.eq("able_bodied")
    amputee_subjects = sorted({subject for subject in subject_keys[amputee_mask].astype(str) if subject})
    if amputee_subjects:
        for subject_key in amputee_subjects:
            test_mask = subject_keys == subject_key
            train_able_mask = able_mask
            train_mixed_mask = ~test_mask
            train_amp_mask = amputee_mask & (~test_mask)
            for prefix, train_mask in {
                "j1_able_to_amputee": train_able_mask,
                "j1_mixed_to_amputee": train_mixed_mask,
                "j1_amputee_loso": train_amp_mask,
            }.items():
                train_idx = metadata.index[train_mask].to_list()
                test_idx = metadata.index[test_mask].to_list()
                if not train_idx or not test_idx:
                    continue
                assert_no_leakage(metadata.assign(_j1_subject_key=subject_keys), train_idx, test_idx, ["_j1_subject_key"])
                splits.append(
                    Split(
                        split_id=f"{prefix}|{subject_key}",
                        train_index=train_idx,
                        test_index=test_idx,
                        train_groups={"subject_id": set(subject_keys.iloc[train_idx].astype(str))},
                        test_groups={"subject_id": {subject_key}},
                    )
                )

    day_keys = _composite_group_series(metadata, "day")
    distinct_days = _valid_group_values(day_keys)
    if len(distinct_days) > 1:
        for day_index, day_key in enumerate(distinct_days):
            if day_index == 0:
                continue
            previous_days = set(distinct_days[:day_index])
            train_idx = metadata.index[day_keys.isin(previous_days)].to_list()
            test_idx = metadata.index[day_keys == day_key].to_list()
            if train_idx and test_idx:
                splits.append(
                    Split(
                        split_id=f"j1_forward_day_logo|{day_key}",
                        train_index=train_idx,
                        test_index=test_idx,
                        train_groups={"day": set(day_keys.iloc[train_idx].astype(str))},
                        test_groups={"day": {day_key}},
                    )
                )
    else:
        session_keys = _composite_group_series(metadata, "session")
        distinct_sessions = _valid_group_values(session_keys)
        if len(distinct_sessions) > 1:
            for session_index, session_key in enumerate(distinct_sessions):
                if session_index == 0:
                    continue
                previous_sessions = set(distinct_sessions[:session_index])
                train_idx = metadata.index[session_keys.isin(previous_sessions)].to_list()
                test_idx = metadata.index[session_keys == session_key].to_list()
                if train_idx and test_idx:
                    splits.append(
                        Split(
                            split_id=f"j1_forward_session_logo|{session_key}",
                            train_index=train_idx,
                            test_index=test_idx,
                            train_groups={"session": set(session_keys.iloc[train_idx].astype(str))},
                            test_groups={"session": {session_key}},
                        )
                    )

    distinct_subjects = _valid_group_values(subject_keys)
    if len(distinct_subjects) > 1:
        for subject_key in distinct_subjects:
            train_idx = metadata.index[subject_keys != subject_key].to_list()
            test_idx = metadata.index[subject_keys == subject_key].to_list()
            if not train_idx or not test_idx:
                continue
            assert_no_leakage(metadata.assign(_j1_subject_key=subject_keys), train_idx, test_idx, ["_j1_subject_key"])
            splits.append(
                Split(
                    split_id=f"j1_subject_logo|{subject_key}",
                    train_index=train_idx,
                    test_index=test_idx,
                    train_groups={"subject_id": set(subject_keys.iloc[train_idx].astype(str))},
                    test_groups={"subject_id": {subject_key}},
...
def _metadata_string_series(metadata: pd.DataFrame, column: str) -> pd.Series:
    if column not in metadata.columns:
        return pd.Series([""] * len(metadata), index=metadata.index, dtype=object)
    series = metadata[column].astype("object").where(metadata[column].notna(), "")
    return series.astype(str)


def _valid_group_values(series: pd.Series) -> list[str]:
    values = [
        value
        for value in series.astype(str).tolist()
        if value and value.lower() not in {"nan", "none", "<na>", "<missing>"}
    ]
    unique_values = sorted(set(values))
    return sorted(unique_values, key=lambda value: _group_sort_key(value))
...
def _group_sort_key(value: str) -> tuple[int, str]:
    ...
    if suffix.startswith("D"):
        try:
            return (_day_sort_key(suffix), str(value))
        except ValueError:
            pass
    return (10**9, str(value))


def _composite_group_series(metadata: pd.DataFrame, column: str) -> pd.Series:
    base = _metadata_string_series(metadata, column)
    source = _metadata_string_series(metadata, "source_dataset_id")
    if len(_valid_group_values(source)) <= 1 or column == "source_dataset_id":
        return base
    combined = [
        f"{source_value}::{base_value}" if base_value else ""
        for source_value, base_value in zip(source.tolist(), base.tolist(), strict=True)
    ]
    return pd.Series(combined, index=metadata.index, dtype=object)
